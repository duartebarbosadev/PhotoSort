...
if __name__ == '__main__':
    ...
    pipeline = ImagePipeline(
        thumbnail_cache_dir="test_image_pipeline_thumb_cache",
        preview_cache_dir="test_image_pipeline_preview_cache"
    )

    # Create dummy files
    sample_raw_file = "dummy_pipeline_test.arw" # Needs to be a real raw for actual processing
    sample_jpg_file = "dummy_pipeline_test.jpg"
    
    if not os.path.exists(sample_raw_file): open(sample_raw_file, 'w').close() # Empty file
    if not os.path.exists(sample_jpg_file): Image.new("RGB", (100,100)).save(sample_jpg_file)

    print(f"--- Testing ImagePipeline ---")

    # Test Thumbnail QPixmap (JPG)
    print(f"\nGetting thumbnail for JPG: {sample_jpg_file}")
    thumb_qpix = pipeline.get_thumbnail_qpixmap(sample_jpg_file, apply_auto_edits=False)
    if thumb_qpix:
        print(f"  JPG Thumbnail QPixmap size: {thumb_qpix.size()}")
    else:
        print(f"  Failed to get JPG thumbnail QPixmap.")

    # Test Preview QPixmap (JPG) - on demand
    print(f"\nGetting preview for JPG: {sample_jpg_file} (max size 800x600)")
    preview_qpix = pipeline.get_preview_qpixmap(sample_jpg_file, (800,600), apply_auto_edits=False)
    if preview_qpix:
        print(f"  JPG Preview QPixmap size: {preview_qpix.size()}")
    else:
        print(f"  Failed to get JPG preview QPixmap.")
    
    # Test Preloading (just paths for now, as dummy raw won't process)
    all_test_files = [sample_jpg_file, sample_raw_file, "non_existent_file.xyz"]
    
    def simple_progress(current, total):
        print(f"  Preload Progress: {current}/{total}")

    print(f"\nPreloading thumbnails for: {all_test_files}")
    pipeline.preload_thumbnails(all_test_files, apply_auto_edits=False, progress_callback=simple_progress)
    
    print(f"\nPreloading previews for: {all_test_files}")
    pipeline.preload_previews(all_test_files, apply_auto_edits=True, progress_callback=simple_progress)

    # Test getting PIL for processing (JPG)
    print(f"\nGetting PIL for processing (JPG): {sample_jpg_file}")
    pil_process_jpg = pipeline.get_pil_image_for_processing(sample_jpg_file, target_mode="L", use_preloaded_preview_if_available=True)
    if pil_process_jpg:
        print(f"  JPG PIL for processing size: {pil_process_jpg.size}, mode: {pil_process_jpg.mode}")
    else:
        print(f"  Failed to get JPG PIL for processing.")

    # Test getting PIL for processing (RAW) - will likely fail with dummy, but tests path
    print(f"\nGetting PIL for processing (RAW): {sample_raw_file}")
    pil_process_raw = pipeline.get_pil_image_for_processing(sample_raw_file)
    if pil_process_raw:
        print(f"  RAW PIL for processing size: {pil_process_raw.size}, mode: {pil_process_raw.mode}")
    else:
        print(f"  Failed to get RAW PIL for processing (expected for dummy file).")


    # Clean up
    print("\nCleaning up test caches and files...")
    pipeline.clear_all_image_caches()
    pipeline.thumbnail_cache.close() # Ensure closed before rmtree
    pipeline.preview_cache.close()   # Ensure closed before rmtree
    
    import shutil
    if os.path.exists("test_image_pipeline_thumb_cache"): shutil.rmtree("test_image_pipeline_thumb_cache")
    if os.path.exists("test_image_pipeline_preview_cache"): shutil.rmtree("test_image_pipeline_preview_cache")
    if os.path.exists(sample_raw_file): os.remove(sample_raw_file)
    if os.path.exists(sample_jpg_file): os.remove(sample_jpg_file)
    
    print("--- ImagePipeline Tests Complete ---")