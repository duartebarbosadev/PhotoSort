...
if __name__ == '__main__':
    ...
    # Dummy application to run the event loop if needed for signals
    app = QApplication(sys.argv)

    # Example file list (replace with actual image paths)
    test_files = [
        # Add paths to some test images here
        # "/path/to/image1.jpg",
        # "/path/to/image2.png",
    ]

    if not test_files:
        print("Please add image paths to the test_files list in similarity_engine.py for testing.")
    else:
        engine = SimilarityEngine()
        
        # Connect signals to simple print functions for testing
        engine.progress_update.connect(lambda p, msg: print(f"Progress: {p}% - {msg}"))
        engine.embeddings_generated.connect(lambda embeds: print(f"Embeddings Generated: {len(embeds)} items"))
        engine.clustering_complete.connect(lambda clusters: print(f"Clustering Complete: {clusters}"))
        engine.error.connect(lambda err: print(f"Error: {err}"))

        # Run in a separate thread (recommended)
        thread = QThread()
        engine.moveToThread(thread)
        thread.started.connect(lambda: engine.generate_embeddings_for_files(test_files))
        engine.embeddings_generated.connect(thread.quit) # Quit thread after embeddings
        engine.error.connect(thread.quit) # Quit on error too

        print("Starting similarity engine thread...")
        thread.start()
        
        # Keep app running until thread finishes (for testing)
        app.exec() 
        print("Test finished.")