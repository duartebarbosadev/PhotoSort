...
# Global instance (optional, can be managed by an application context)
# For direct use similar to the original ImageHandler:
# thumbnail_cache_instance = ThumbnailCache()

if __name__ == '__main__':
    # Example Usage
    cache = ThumbnailCache(cache_dir=os.path.join(os.path.expanduser('~'), '.cache', 'test_phototagger_thumbnails'))
    print(f"Initial cache volume: {cache.volume() / 1024:.2f} KB")

    # Create a dummy image
    try:
        dummy_image = Image.new('RGB', (60, 30), color = 'red')
        dummy_image_path = "/test/image1.jpg"
        dummy_apply_edits = False
        cache_key = (dummy_image_path, dummy_apply_edits)
        ...
        if retrieved_image:
            ...
            assert retrieved_image.size == (60, 30)
        else:
        ...
        if cache_key not in cache:
            print(f"Key {cache_key} successfully deleted.")
        else:
            print(f"Key {cache_key} still exists after delete (ERROR).")
        
        print(f"Cache volume after deleting item: {cache.volume() / 1024:.2f} KB")

    except Exception as e:
        print(f"Error during ThumbnailCache test: {e}")
    finally:
        # Clean up test cache
        cache.clear()
        cache.close()
        import shutil
        if os.path.exists(os.path.join(os.path.expanduser('~'), '.cache', 'test_phototagger_thumbnails')):
             shutil.rmtree(os.path.join(os.path.expanduser('~'), '.cache', 'test_phototagger_thumbnails'))
        print("Test ThumbnailCache cleaned up.")