...
class MenuManager:
    ...
    def _create_actions(self):
        ...
        self.rotate_counterclockwise_action = QAction("Rotate Counterclockwise", main_win)
        ...
        self.clear_marked_deletions_action = QAction("Clear All Deletion Marks", main_win)
    ...
    def _create_settings_menu(self, menu_bar):
        ...
        self.toggle_mark_for_deletion_action = QAction("Mark for Deletion (vs. Direct Delete)", main_win)
        self.toggle_mark_for_deletion_action.setCheckable(True)
        self.toggle_mark_for_deletion_action.setChecked(main_win.mark_for_deletion_mode_enabled)
    ...
    def connect_signals(self):
        ...
        # View Menu
        self.toggle_folder_view_action.toggled.connect(main_win._toggle_folder_visibility)
        self.group_by_similarity_action.toggled.connect(main_win._toggle_group_by_similarity)
        ...
        # Settings Menu
        self.manage_cache_action.triggered.connect(self.dialog_manager.show_cache_management_dialog)
        self.toggle_auto_edits_action.toggled.connect(main_win._handle_toggle_auto_edits)
        self.toggle_mark_for_deletion_action.toggled.connect(main_win._handle_toggle_mark_for_deletion_mode)

        # Image Menu
        self.rotate_clockwise_action.triggered.connect(main_win._rotate_current_image_clockwise)
        self.rotate_counterclockwise_action.triggered.connect(main_win._rotate_current_image_counterclockwise)
        self.rotate_180_action.triggered.connect(main_win._rotate_current_image_180)
        self.mark_for_delete_action.triggered.connect(main_win._mark_selection_for_deletion)
        self.unmark_for_delete_action.triggered.connect(main_win._mark_selection_for_deletion) # Same action toggles
        self.commit_deletions_action.triggered.connect(main_win._commit_marked_deletions)
        self.clear_marked_deletions_action.triggered.connect(main_win._clear_all_deletion_marks)
    ...
    def update_recent_folders_menu(self):
        ...
        if not hasattr(self, 'open_recent_menu'): return
        ...
        if not recent_folders:
            ...
            for folder in recent_folders:
                ...
                action.triggered.connect(lambda checked=False, f=folder: self.main_window.app_controller.load_folder(f))
                self.open_recent_menu.addAction(action)

    def show_image_context_menu(self, position: QPoint):
        main_win = self.main_window
        active_view = main_win.sender()
        if not hasattr(active_view, 'indexAt'): return

        proxy_index = active_view.indexAt(position)
        if not main_win._is_valid_image_item(proxy_index): return
        ...
        if MetadataProcessor.is_rotation_supported(file_path):
            ...
            rotate_cw.triggered.connect(lambda: main_win._rotate_selected_images('clockwise'))
            rotate_ccw.triggered.connect(lambda: main_win._rotate_selected_images('counterclockwise'))
            rotate_180.triggered.connect(lambda: main_win._rotate_selected_images('180'))
        ...
        # Explorer
        show_in_explorer = QAction(QIcon.fromTheme("folder-open", main_win.style().standardIcon(QStyle.StandardPixmap.SP_DirOpenIcon)), "Show in Explorer", main_win)
        show_in_explorer.triggered.connect(lambda: self._open_image_in_explorer(file_path))
    ...
    def _open_image_in_explorer(self, file_path: str):
        try:
            ...
            logging.error(f"Error showing '{file_path}' in explorer: {e}", exc_info=True)