...
@pytest.mark.skipif(not IMPORTS_AVAILABLE, reason=f"Required modules not available: {IMPORT_ERROR}")
class TestImageRotator:
    ...
    def setup_class(cls):
        ...
        if os.path.exists(cls.test_folder):
            for filename in os.listdir(cls.test_folder):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.arw', '.cr2', '.nef')):
    ...
    def test_calculate_new_orientation(self):
        ...
        orientation = 1
        for _ in range(4):
            orientation = self.rotator._calculate_new_orientation(orientation, 'clockwise')
    ...
    def test_rotation_error_handling(self):
        ...
        success, message = self.rotator.rotate_image("/nonexistent/file.jpg", 'clockwise')
        ...
        if self.sample_images:
            ...
            success, message = self.rotator.rotate_image(temp_image, 'invalid_direction')
            assert success == False
...
class TestMetadataProcessorRotation:
    ...
    def test_rotate_clockwise(self):
        ...
        success = MetadataProcessor.rotate_clockwise(temp_image, exif_disk_cache=self.exif_cache)
    ...
    def test_rotate_counterclockwise(self):
        ...
        success = MetadataProcessor.rotate_counterclockwise(temp_image, exif_disk_cache=self.exif_cache)
    ...
    def test_rotate_180(self):
        ...
        success = MetadataProcessor.rotate_180(temp_image, exif_disk_cache=self.exif_cache)
...
class TestRotationIntegration:
    ...
    def test_mixed_rotations(self):
        ...
        for rotate_func, description in rotations:
            ...
            assert os.path.exists(temp_image), f"File missing after {description} rotation"
