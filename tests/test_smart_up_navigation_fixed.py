...
def test_navigate_up_is_sequential(tmp_path):
    ...
    # Expand the group and select middle item (b)
    ...
    # Up should be sequential: move to previous (a)
    mw.navigate_up_sequential(skip_deleted=True)
    
    sel_model = mw._active_file_view.selectionModel()
    ...
    # Up again from first: should go to header or stay on a; verify it doesn't wrap within group
    mw.navigate_up_sequential(skip_deleted=True)
    sel_model2 = mw._active_file_view.selectionModel()
    ...
    txt = mw.file_system_model.itemFromIndex(cur_idx2).text()
    assert txt in {"a.jpg", "Group 1"}
