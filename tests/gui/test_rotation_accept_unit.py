...
pytest.importorskip(
    "PyQt6.QtWidgets", reason="PyQt6 not available for GUI tests", exc_type=ImportError
)
...
def test_accept_single_rotation_moves_to_next(qapp, tmp_path):
    ...
    if hasattr(window, "_select_items_in_current_view"):
        window._select_items_in_current_view([target])
    else:
        # Fallback: directly manipulate selection model if helper absent
        window._reselect_paths_after_view_rebuild([target])  # type: ignore[attr-defined]
...
def test_accept_multi_non_contiguous_next_selection(qapp, tmp_path):
    ...
    if hasattr(window, "_select_items_in_current_view"):
        ...
        window._reselect_paths_after_view_rebuild(targets)  # type: ignore[attr-defined]
