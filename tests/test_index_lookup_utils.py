...
from src.ui.helpers.index_lookup_utils import find_proxy_index_for_path, classify_selection
...
def test_find_proxy_index_basic(tmp_path):
    ...
    proxy = IdentityProxy(); proxy.setSourceModel(src)
    view = QTreeView(); view.setModel(proxy)
    ...
    idx = find_proxy_index_for_path(str(p2), proxy, src, lambda i: _is_valid(i, src), view.isExpanded)
    assert idx.isValid()
...
def test_find_proxy_index_missing(tmp_path):
    ...
    p1 = tmp_path / 'a.jpg'; p1.write_text('x')
    src.appendRow(make_item(str(p1)))
    idx = find_proxy_index_for_path(str(tmp_path / 'missing.jpg'), proxy, src, lambda i: _is_valid(i, src), view.isExpanded)
