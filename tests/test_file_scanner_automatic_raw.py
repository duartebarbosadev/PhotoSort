...
class TestFileScannerAutomaticRaw:
    ...
    def test_scan_directory_blur_detection_without_apply_auto_edits(self, mock_blur, mock_isfile, mock_walk):
    ...
    @patch('src.core.file_scanner.os.walk')
    @patch('src.core.file_scanner.os.path.isfile')
    def test_preload_thumbnails_called_without_apply_auto_edits(self, mock_isfile, mock_walk):
    ...
    def test_async_scan_directory_blur_detection_signature(self):
        ...
        with patch('src.core.file_scanner.BlurDetector.is_image_blurred') as mock_blur, \
            ...
            is_blurred = scanner.__class__.__dict__['_scan_directory_async'].__code__.co_varnames
            ...
            with patch.object(BlurDetector, 'is_image_blurred') as mock_blur_method:
                ...
                result = BlurDetector.is_image_blurred(test_path, threshold=100.0)
...
class TestFileScannerIntegration:
    ...
    def test_complete_scan_workflow_without_raw_params(self):
        ...
        with patch('src.core.file_scanner.os.walk') as mock_walk, \
            ...
            # Setup test data
            mock_walk.return_value = [("/test", [], ["image.jpg", "raw.arw", "other.txt"])]
            mock_isfile.return_value = True
            mock_blur.return_value = False
