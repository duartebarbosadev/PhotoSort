...
class TestAutomaticRawProcessing:
    ...
    def test_raw_extension_detection(self):
        ...
        for ext in raw_extensions:
            ...
            assert is_raw_extension(ext.upper()), f"Extension {ext.upper()} should be detected as RAW"
    ...
    def test_non_raw_extension_detection(self):
        ...
        for ext in non_raw_extensions:
            assert not is_raw_extension(ext), f"Extension {ext} should NOT be detected as RAW"
            # Test case insensitive
            assert not is_raw_extension(ext.upper()), f"Extension {ext.upper()} should NOT be detected as RAW"
    ...
    def test_internal_raw_detection_in_get_pil_thumbnail(self):
        ...
        with patch('src.core.image_pipeline.is_raw_extension') as mock_is_raw:
            ...
            with patch('os.path.exists', return_value=True), \
                 patch('src.core.image_processing.raw_image_processor.RawImageProcessor.process_raw_for_thumbnail') as mock_process:
                 
                mock_process.return_value = Mock()  # Mock PIL image
                
                # Call the internal method directly
                result = pipeline._get_pil_thumbnail("test.arw")
                
                # Verify that is_raw_extension was called (can be called multiple times internally)
                mock_is_raw.assert_called_with(".arw")
                assert mock_is_raw.call_count >= 1, "is_raw_extension should be called at least once"
    ...
    def test_cache_key_generation_includes_raw_detection(self):
        ...
        with patch('src.core.image_pipeline.is_raw_extension') as mock_is_raw, \
            ...
            assert cache_key[1] == True  # RAW detection result for .arw
            assert cache_key[2] == True  # orientation
    ...
    def test_preload_thumbnails_processes_each_file_individually(self):
        ...
        with patch.object(pipeline, '_get_pil_thumbnail') as mock_get_pil, \
...
class TestRawProcessingIntegration:
    ...
    def test_pipeline_initialization_without_auto_edits_params(self):
        ...
        try:
            pipeline.get_preview_qpixmap("nonexistent.cr2", display_max_size=(1000, 1000))
        except Exception:
            pass  # Expected to fail due to missing file
