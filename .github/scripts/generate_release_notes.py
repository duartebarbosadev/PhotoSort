...
class ReleaseNotesGenerator:
    ...
    def get_commits_with_pr_info(self, from_tag: Optional[str], to_tag: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if from_tag:
                ...
                result = subprocess.run(
                    ["git", "log", f"--pretty=format:%H|%s|%b{separator}", "--no-merges", f"{from_tag}..{to_tag}"],
                    capture_output=True,
                    text=True,
                    check=True,
                )
                ...
                # First release - get all commits
                result = subprocess.run(
                    ["git", "log", f"--pretty=format:%H|%s|%b{separator}", "--no-merges", to_tag],
                    capture_output=True,
                    text=True,
                    check=True,
                )
                ...
                for block in commit_blocks:
                    ...
                    if lines and '|' in lines[0]:
                        ...
                        if len(parts) >= 2:
                            ...
                            if len(lines) > 1:
                                additional_lines = [line for line in lines[1:] if line.strip()]
                                if additional_lines:
                                    if body:
    ...
    def generate_release_notes(self, commits: List[Dict[str, Any]], tag_name: str) -> str:
        ...
        for commit in commits:
            ...
            if commit['pr_number']:
                ...
                pr_links.append(f"- PR #{commit['pr_number']}: {commit['pr_title'] or commit['subject']}")
...
"""
    ...
    def _create_fallback_notes_from_commits(self, commits: List[Dict[str, Any]], tag_name: str) -> str:
        ...
        for commit in commits:
            ...
            if commit['pr_number']:
                ...
                pr_section.append(f"- PR #{commit['pr_number']}: {commit['pr_title'] or commit['subject']}")
