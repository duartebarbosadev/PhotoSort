...
- **Metadata I/O (pyexiv2)**: All reads/writes of EXIF/XMP/IPTC MUST go through `src/core/metadata_io.py` (class `MetadataIO`). Do not import `pyexiv2` directly outside this module. Higher-level flows (e.g., `MetadataProcessor`, `ImageRotator`) should delegate to `MetadataIO` for tag reads/writes and basic properties to ensure thread-safety and consistent behavior across platforms/frozen builds.
